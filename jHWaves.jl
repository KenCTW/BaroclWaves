...
This collection seeks only a real frequency.
...
Translated from Matlab bigr___ code  (5/21/2018 version)

K.H. Brink  4/20/2026
"""

"""
	Function jHWavesM(arr)
...
#       Allows for real frequency only.
#       Modes are not orthogonal except in the inviscid, long wave, no-mean-flow limit.       
#       Calls various other functions beginning with "jHWaves"
#
#   Call as 
#       jHWavesM(arr)
#   where
...
#
#   See function jHWavesSetup to help set this up, and jHWavesFinch to change the input
#       tuple conveniently
...
#       wz          where wz is the first guess frequency (rad/sec)
...
#       ilong       ilong = 1: general frequency and wavenumber, ilong = 0, long wave limit
...
#                      If vzero = 0, mean flow is zero everywhere, and the
#                        remaining information is not needed. 
...
#       The grid points are arranged so that it is preferable to have nn > mm
	...
	global nn, mm, dt, dx, f, rl, del, icbc, iobc, BB, h, ilong
	...
	println(' ')
	if ilong < 0.5
    		println("Coastal long wave limit")
	end
	if del > 0.5
	...
	#    Get the command on pausing during execution or not
	ipause = arr[32]
...
#   Now actually do the calculations
	...
	while ncal <= npts
		...
		NLopt.min_objective!(opt,jHWavesDrvr)
		NLopt.xtol_rel!(opt,eps)
		NLopt.maxeval!(opt,maxit)
		NLopt.initial_step!(opt, steppi)
...
#		Calculate and plot pressure
        		...
        		wj = jHWavesDrvr(www,0.)
			csign = BB[(2*mm-1)]/abs(BB[(2*mm -1)])
			...
			if ipause > 0.5				
				...
				contour!(ax1, xgr,zgr,p, levels = 10, color = :black)	
				contour!(ax1,xgr,zgr,p,levels = vec([0 0]), linewidth = 3, color = :black)
	...
	if exitflag == 1
		println(' ')
		jHWavesEngDiag(wnn[ncal-1,1],BB)
	end


#	Plot final dispersion curve with perturbation damping
	...
	if npts > 1.5 
		...
		lines!(ax2,rll,wnn[:,1])
		ax2.ylabel = "ω [1/s]"
		ax2.xlabel = "Wavenumber [1/cm]"
		ax2.title = "Dispersion Curve"
		xlims!(ax2,0.,maximum(rll))
		hidedecorations!(ax2; label = false, ticklabels = false, ticks = false )

		lines!(ax1,rll,wnn[:,2])
		ax1.ylabel = "1/T_f [1/s]"
		ax1.xlabel = "Wavenumber [1/cm]"
		ax1.title = "Perturbation Damping Rate"
		xlims!(ax1,0.,maximum(rll))
		hidedecorations!(ax1; label = false, ticklabels = false, ticks = false )
	...
	isave = inputi("Do you want to save these results? 0 (no) or 1 (yes) ")
		...
		verbiage = """
			Contents of the tuple:
				...
				2) ilong (= 0 for long wave, = 1 for general)
				...
				6) rll (wavenumber)
				7) wnn (real frequency and perturbation damping rate)
				8) xgr (x grid in km)
				9) zgr (z grid in m)
				10) p  (pressure)
...
#       Offshore, top
	...
	aa4 = [aa3, 0, aa1]*wf2/n2[nn-2,mm-2]
...
"""
	...
	Compute coefficients for the x = 0 boundary condition

K.H. Brink 11/28/2025
"""


function jHWavesCc(w)

#   Compute coefficients for coastal boundary condition (avoiding very top and bottom grid points)
#  
#  	call as
#		cc = jHWavesCc(w)
#   For use with real frequency only
#
#	K.H. Brink 11/28/2025 based on Matlab version of 6/11/2018

	global rl, m2, n2, f, vm, h, mm, vmx, vmxx, icbc, dx, dt, hx, m2x, m2z, ilong, iobc
	...
	fsw = f*fs - wps
	fswx = f*vmxx[1,:] - 2*m2x[1,:].*m2n2 + (m2z[1,:].*m2n2).*m2n2 - ilong*2*rl*wp.*vmx[1,:]
...
"""
	...
	Compute coefficients for offshore boundary condition
...
function jHWavesOff(w)

#   Compute the coefficients for the offshore boundary condition
#	call as
#		cc = jHWavesOff(w)
#
#	K.H. Brink, 11/28/2025, based on Matlab code of 12/23/2004

	global rl, m2, n2, f, vm, h, hx, nn, mm, vmx, vmxx, dx, dt, hx, m2x, m2z, ilong, iobc
	...
	if iobc > 0.5
#   open boundary at x = xmax
    
		fswx = f*vmxx[nn-2,:] - ilong*2*rl*wp.*vmx[nn-2,:] -2*m2x[nn-2,:].*m2n2 + mmmm
...
"""
	...
	Function to compute coefficients for the bottom boundary condition
...
function jHWavesBot(w)

#   Compute coefficients for bottom boundary condition
#	call as
#		cc = jHWavesBot(w)
#
#  K.H. Brink 11/28/2025, based on Matlab of 6/11/2018

	global nn, dx, dt, m2, m2x, m2z, n2, n2z, f, rl, vm, vmx, vmxx, h, hx, hxx, ilong
	...
	fspw = f*fs + wps
	rfsw = 0.0
	fswx = f*fsx -ilong*2*rl*vmx[:,1].*wp
	...
	fswbx = f*fsbx - ilong*2*rl*wp.*vmbx
	...
	b3 = wp.*m2n2 -wp.*hx 
	b4 = 0.0
	b5 = -(wp./n2[:,1]).*(f*fp - ilong*wp.*wp)  + (wp.*m2n2).*hx
	b6 = rl*f*m2n2 - rl*f*hx
...
#  K.H. Brink 11/28/2025 based on Matlab of 12/23/2004
	...
	global del, nn, mm, f, rl, vm, m2, n2, vmx, h, dx, dt, ilong
...
#	K.H. Brink 11/29/2025
	...
	global nn, mm, dt, dx, f, rl, vm, vmx, vmxx, m2, n2, n2z, m2z, m2x, h, hx, hxx, ilong
	...
	fswx = f*fsx -ilong*2*rl*wp.*vmx[n,:]


	fswz = m2x[n,:]- ilong*2*rl*wp.*m2[n,:]/f - 2*m2z[n,:].*m2n2 + ((m2n2.*m2n2).*n2z[n,:])
	...
	a1 = -wp
	a2 = 2*m2n2.*wp
	a3 = -(wp.*(f*fp - ilong*wps))./n2[n,:]
	a4 = wp.*rq + wp.*m2n2z
	a5 = -(wp.*m2n2).*rq + rl*vmx[n,:].*m2n2 + wp.*m2n2x + (rl/f)*m2n2.*(f*f*ones(mm-2) - ilong*wps)
	a5 = a5 - (rl*(m2[n,:]/f)./n2[n,:]  - (wp.*n2z[n,:])./(n2[n,:].*n2[n,:])).*(f*fp - ilong*wps)
	a5 = a5 - (wp./n2[n,:]).*(m2z[n,:].*m2n2 + m2[n,:].*m2n2z)
	a6 = rl*f*rq + ilong*wp*rl*rl + rl*f*m2n2z
...
#  K.H. Brink 2/5/2026, based on matlab of 5/22/2018
	...
	for n = 1:(nn-2)
		...
		im = (irow) - (1*ones(Int,mm-2))
    		pt = (BB[ip] - BB[im])/dt2
    		pxp = (BB[(irow+mm*ones(Int,mm-2))] - BB[(irow-mm*ones(Int,mm-2))])/dx2
       
    		px = pxp + thx.*pt
    		pz = pt/h[n]
		vt = fs.*px + ilong*rl*wp.*p - ilong*(wp.*wp.*m2n2).*pz/f
		v[n,:] = vt./fsw
		
    		wvel[n,:] = -(m2n2./fsw).*(-wp.*px - rl*f*p)
...
#		plot u
	...
	contour!(ax1,xgr,zgr,real(u), levels = 5, color = :red)
	contour!(ax1,xgr,zgr,real(u),levels = vec([0 0]), linewidth = 2, color = :red)
...
#		plot v
	...
	contour!(ax2,xgr,zgr,real(v), levels = 5, color = :black)
...
#		plot w
	...
	contour!(ax3,xgr,zgr,real(wvel), levels = 5, color = :red)
	contour!(ax3,xgr,zgr,real(wvel),levels = vec([0 0]), linewidth = 2, color = :red)
...
#		plot rho
	...
	contour!(ax4,xgr,zgr,real(rho), levels = 5, color = :black)
...
#		plot surface p
	lines!(ax1,vec(xdep),vec(ptop/ssff), color = :black)
	lines!(ax1,vec(xdep),vec(0*xdep),color = :black)
	xlims!(ax1,0,xmax)
	hidedecorations!(ax1, grid = true, ticklabels = false, label = false)
 	ax1.ylabel = "p [dyne/cm^2]"
	ax1.xlabel = "x [km]"
	ax1.title = "Surface Pressure (all units arbitrary, but consistent)"


#		plot v
	lines!(ax2,vec(xdep),vec(v[:,end]/ssff), color = :black)
...
#		plot u
	lines!(ax3,xdep,real(u[:,end]/ssff), color = :black)
...
function jHWavesEngDiag(war,BB)

#  Compute energy diagnistics: most useful for unstable waves
#	call as
#		jHWavesEngDiag(war,BB)
#
#  K.H. Brink 12/1/2025bade on Matlab of 6/11/2018


	global nn, mm, dx, dt, f, rl, del, r, rx, h, hx, hxx, n2, m2, m2x, m2z, n2z, vm, vmx, vmxx, ilong
...
"""
	...
	For the coastal long wave limit only: Compute long wave coefficients

K.H. Brink	12/1/2025
"""

function jHWavesLong(w,BB)

#   Calculate long wave coefficients, valid only in long wave limit with vzero = 0
#
#   Call as
#       BB = jHWavesLong(w,BB)
#   where
#       w = frequency
#       BB = pressure as a 1-D array
#   Returns pressure normalized in the long-wave sense
#
#	K.H. Brink 12/1/2025, based on Matlab of 6/11/2018


	global h, hx, f, dt, dx, icbc, iobc, nn, mm, rl, r, ilong

#   Normalize modal structure

	if icbc < 0.5 || iobc< 0.5
		dz = h[1]*dt
		nlow = mm+2
		nhigh = 2*mm - 1
		xx = (1-icbc)*dz*(sum(BB[nlow:nhigh].^2) - 0.5*(BB[nlow]^2  + BB[nhigh]^2))
		dz = h[nn-2]*dt
		nlow = nn*mm - 2*mm + 2
		nhigh = nn*mm - mm - 1
		xxo = (1-iobc)*dz*(sum(BB[nlow:nhigh].^2) - 0.5*(BB[nlow]^2  + BB[nhigh]^2))
		nlow = mm+2
		nhigh = nn*mm - 2*mm + 2
		yy = dx*sum(hx.*(BB[nlow:mm:nhigh].^2)) -0.5*dx*(hx[1]*BB[nlow]^2 + hx[nn-2]*BB[nhigh]^2)

		rnorm = sqrt(abs((xx - xxo + yy )/f))
		BB = BB/rnorm

#   Compute frictional damping term
		...
		dt2 = 2*dt
    		...
    		pbx = (BB[nlow+mm:mm:nhigh+mm] - BB[nlow - mm:mm:nhigh - mm])/dx2
    
    		pt = (BB[nlow+1:mm:nhigh+1] - BB[nlow - 1:mm:nhigh - 1])/dt2
    		thx = hx./h
    
    		pxb = pbx + thx.*pt
    		zz = r.*(pbx.*pxb)
 
    		fricint = dx*(sum(zz) - 0.5*(zz[1] + zz[nn-2]))
    
		cn = w/rl; 
		bn = -((1-icbc)*BB[2*mm-1] - (1-iobc)*BB[nn*mm-mm-1])/f
		ann = fricint/(f*f)

		println(' ')
		println("Long wave phase speed, 		   cn (cm/sec) = ", cn)		
		println("Long wave coupling coefficient, bn ((cm / sec)^-0.5)) = ", bn)
		println("Long wave damping term, 		   ann (cm^-1) = ", ann)

	else
    		println("Will not compute long wave parameters with two open boundaries")
	end
	return BB
end								# end of function
...
"""
	...
	Compute perturbation estimate of damping time

K.H. Brink 12/1/2025
"""

function jHWavesTf(w,BB)

#   Calculate and display Tf (frictional decay time) valid only for vzero = 0
#   Call as
#       w1 = jHWavesTf(w,BB)
#   Where 
#       w = frequency
#       BB = pressure as a 1-D array
#
#  K.H. Brink 12/1/2025 based on 3/15/2005 Matlab


	global h, hx, iobc, icbc, ilong, f, dx, dt, nn, mm, rl, n2, r, del
	...
	fb = BB[(mm+2:mm:nn*mm-2*mm+2)]
	fbx = (BB[(2*mm+2:mm:nn*mm -mm + 2)]- BB[(2:mm:nn*mm - 3*mm + 2)])/dx2
	fbt = (BB[(mm+3:mm:nn*mm - 2*mm + 3)]-BB[(mm+1:mm:nn*mm-2*mm + 1)])/dt2
	thxb = hx./h
	fxb = fbx + thxb.*fbt

	fc = BB[(mm+2:2*mm-1)]
	fo = BB[(nn*mm-2*mm+2:(nn*mm - mm - 1))]
	fs = BB[(2*mm-1:mm:nn*mm-mm - 1)]

	ri11 = dx*(sum(hx.*(fb.*fb)) - 0.5*(hx[1]*fb[1]*fb[1] + hx[nn-2]*fb[nn-2]*fb[nn-2]))
	ri12 = (1-icbc)*h[1]*dt*(sum(fc.*fc) - 0.5*(fc[1]*fc[1] + fc[mm-2]*fc[mm-2]))
	ri13 = (1-iobc)*h[nn-2]*dt*(sum( fo.*fo) - 0.5*(fo[1]*fo[1] + fo[mm-2]*fo[mm-2]))
	ri14 = ilong*del*dx*(sum(fs.*fs) - 0.5*(fs[1]*fs[1] + fs[nn-2]*fs[nn-2]))
	ri15 = 0

	for n = 1:nn-2
    		fz = (BB[(n*mm+3:(n+1)*mm)]-BB[(n*mm + 1:((n+1)*mm -2))])/(h[n]*dt2)
    		func = (fz.*fz)./n2[n,:]
    		pint = h[n]*dt*(sum(func) - 0.5*(func[1] + func[mm-2]))
    		dxx = dx
    		if n == 1 | n == nn-2   
        		dxx = dx/2
    		end
	...
	func = r.*(fbx.*fxb + ilong*rl*rl*fb.*fb);
	ri21 = dx*(sum(func) - 0.5*(func[1] + func[nn-2]))
	func = r.*(fb.*(fbx + fxb))
	ri22 = ilong*dx*(sum(func)- 0.5*(func[1] + func[nn-2]))

	r1 = (f*rl/w)*(ri11 + ri12 -ri13) -(2*w*w/g)*ri14 -2*w*w*ri15
	r2 = ((f*f + ilong*w*w)*ri21 + 2*w*rl*f*ri22)/(f*f- ilong*w*w)

	w1 = r2/r1


	println(' ')
	@printf("Perturbation frictional decay time = %.4e sec \n", 1/w1)
	return w1
end						#	End of function
...
function jHWavesNorm(pin)

#   Normalize pressure mode
...
#	K.H. Brink 12/1/2025 based on Matlab of 6/11/2018
	...
	dz = h[1]*dt
	pc = pin[1,:]
	con1 = sum(pc.*pc) - 0.5*(pc[1]^2 + pc[mm-2]^2)
	con1 = con1*dz
...
"""
	...
	Create the input tuple for use with jHSWavesM

K.H. Brink 11/19/2025
"""


function jHWavesSetup()

#   Create an input tuple for "jHWavesM", given inputs
#   Call as
#       arr = jHWavesSetup()
#   In response to queries, you can enter either numbers or array names
#
#   K.H. Brink  11/19/2025  based on Matlab 6/11/2018 version

	println(' ')
	println("      Coastal-Trapped waves with real frequency")
	println(' ')
	...
	wg = inputf("First guess at frequency (rad/sec)? ")
...
"""
	...
	Make changes to input tuple for HYbrid waves with real frequency
	Returns a revised tuple

K.H. Brink 11/21/2025, 4/2/2026
"""


function  jHWavesFinch(arr)

#   Function to modify an existing input file conveniently
#
#   K.H. Brink, 11/21/2025, 4/2/2026 based on Matlab 3/31/2012 version

	println(' ')
	println("     Function to create modified input tuple")
	println(" ")
	...
	if stin == 'g'
    		...
    		narr = (iii[1], iii[2], arr[3:end]...)

	elseif stin == 'w'
    		println("Old frequency guess (sec^-1) ",  arr[3])
    		www = inputf(" Enter new w (sec^-1) ")
		narr = (arr[1:2]..., www, arr[4:end]...)
    
	elseif stin == 'c'
		if arr[7] > 0.5
			println("Previously had general frequency and wavenumber")
		else
			println("Previously had coastal long wave limit")
		end		
 		iii = inputi(" Enter 1 for general frequency, wavenumber, 0 for long wave limit ")
		if abs(arr[24]) > 0.001 && iii[1] == 0
			println(" ")
			println("Error! Can not use the long wave limit with a mean flow!")
			return	
		end
		narr = (arr[1:6]..., iii, arr[8:end]...)
